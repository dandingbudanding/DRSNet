...
class SEResNet(nn.Module):
    def __init__(self,block,num_blocks,in_ch,out_ch,reduction=16):
        ...
        self.layer2 = self._make_layer(block, 128, num_blocks[1], stride=1,reduction=reduction)
        self.layer3 = self._make_layer(block, 256, num_blocks[2], stride=1,reduction=reduction)
        self.layer4 = self._make_layer(block, 512, num_blocks[3], stride=1,reduction=reduction)
        self.up5 = nn.Sequential(
            nn.Conv2d(512, 256, 1),
            nn.BatchNorm2d(256),
            nn.ReLU()
        )
        self.res2to4 = MultiscaleResblock(256, 256)
        ...
        self.conv6 = NeckConv(64, 64)
        self.conv7 = nn.Conv2d(64, out_ch, kernel_size=1, stride=1)
