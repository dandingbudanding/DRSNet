...
class Resnet_Unet(nn.Module):
    ...
    BN_enable控制是否存在BN，定稿设置为True
    """
    def __init__(self, in_ch,out_ch,BN_enable=True, resnet_pretrain=False):
        super().__init__()
        self.BN_enable = BN_enable
        # encoder部分
        ...
        self.firstconv = nn.Conv2d(in_channels=in_ch, out_channels=64, kernel_size=7, stride=2, padding=3, bias=False)
        ...
        if self.BN_enable:
            self.final = nn.Sequential(
                ...
                nn.Conv2d(in_channels=32, out_channels=out_ch, kernel_size=1),
                nn.Sigmoid()
            )
        else:
            self.final = nn.Sequential(
