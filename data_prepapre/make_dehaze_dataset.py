...
if __name__ == '__main__':
    ...
    test_num = int(lenof_src * 0.05)  # 分多少给测试集
    for _,file in enumerate(src_all):
        img=cv2.imread(os.path.join(src_path,file))
        label = cv2.imread(os.path.join(label_path, file[:-3] + "png"), 0)  # 0:origin 1:bgr
        ...
        if _<test_num:
            cv2.imwrite(os.path.join(test_path, str(_) + ".jpg"), img)
            cv2.imwrite(os.path.join(test_path, str(_) + ".bmp"), img_addrain)
            cv2.imwrite(os.path.join(test_path, str(_) + ".png"), label)
        else:
            cv2.imwrite(os.path.join(train_path, str(_-test_num) + ".jpg"), img)
            cv2.imwrite(os.path.join(train_path, str(_-test_num) + ".bmp"), img_addrain)
